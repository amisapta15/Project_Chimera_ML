function [model,train_time,train_accuracy,test_accuracy]=RVFL_train_val_NEW(trainX,trainY,testX,testY,option)
...
%% RVFL_AE part
if strcmp(option.method,'RVFL_AE')
    switch lower(option.ActivationFunction)
        ...
        case {'sign'}
            H = sign(H);
    end %Switch Case Ended
    %% Call Auto-Encoder Function
    Weight  =  sparse_l1_autoencoder(H,trainX,1e-3,50)'; %l1-regulraization
    %clear A1;
    [~,feature_size]=size(trainX);
    Bias=sum(Weight)./feature_size;
    %Bias=zeros(1,N);
    Bias_train=repmat(Bias,Nsample,1);
    H=trainX*Weight+Bias_train;
end
...
%% Testing Phase Starts From here
model.Weight=Weight;
model.Bias=Bias;
model.beta=beta;
model.k=k;
